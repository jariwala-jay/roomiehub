...
const ProfileCard = ({ user, currentUser, matchPercentage }) => {
    ...
    const fetchRequestStatus = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/requests/check",
          {
  ...
  return (
    <>
      <div className="w-full max-w-md mx-auto p-2 border-t-2 border-[#eeeeee] bg-[#ffffff] rounded-2xl drop-shadow-lg">
        <div className="bg-[#eeeeeea7] p-2 rounded-xl">
          <div className="flex items-center relative space-x-4">
            <div className="w-16 h-16 cursor-pointer" onClick={togglePopup}>
              <img
                src={
                  user.profile_pic
        ...
        <div className="flex mt-2 items-center justify-between">
          ...
          <p
            className="text-[#4dc63b] text-sm cursor-pointer"
            onClick={togglePopup}
          >
            View Details
          </p>
      ...
      <ProfileCardPopup user={user} isOpen={popupOpen} onClose={togglePopup} />
    </>
  );
};
